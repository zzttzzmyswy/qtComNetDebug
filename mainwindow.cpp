...
MainWindow::MainWindow(QWidget *parent)
  ...
  ui->comboBoxFlowControl->addItem("NoFlowControl", QSerialPort::NoFlowControl);
  ...
  connect(timerScanCom, SIGNAL(timeout()), this, SLOT(comTimeScanComData()));
  ...
  connect(&netTcpClient,
          static_cast<void (QAbstractSocket::*)(QAbstractSocket::SocketError)>(
              &QAbstractSocket::error),
          this, &MainWindow::netDisplayErrorClient);
#endif
}
...
void MainWindow::on_pushButtonScanCom_clicked() {
  ...
  ui->comboBoxComName->addItem(QString("自定义"));
}
...
void MainWindow::on_pushButtonConnect_clicked() {
  if (ui->pushButtonConnect->text() == QString("打开连接")) {
    if (serial_status) {
      ...
      /* 设置串口号 */
      serial.setPortName(ui->comboBoxComName->currentText());
      /* 设置比特率 */
      serial.setBaudRate(ui->comboBoxBitNum->currentText().toInt());
      /* 设置数据位 */
      serial.setDataBits(
      ...
      serial.setParity(
          QSerialPort::Parity(ui->comboBoxCheckBit->currentData().toInt()));
...
void MainWindow::comSendDataFromTextEdit() {
  ...
  textEditStr +=
      QString(ui->checkBoxEnter->checkState() == Qt::Checked ? "\r\n" : "");
  ...
  case 0: // UTF8
    ...
    break;
  case 1: // GBK
    UnicodesendStr = utf8codec->toUnicode(textEditStr.toLocal8Bit());
...
void MainWindow::netServerAcceptConnection() {
  ...
  if (!netTcpServerConnection) {
    QMessageBox::critical(NULL, "错误", tr("错误: 连接失败!"), QMessageBox::Yes,
                          QMessageBox::Yes);
    return;
  }
  ...
  connect(&netTcpClient,
          static_cast<void (QAbstractSocket::*)(QAbstractSocket::SocketError)>(
              &QAbstractSocket::error),
          this, &MainWindow::netDisplayErrorServer);
#endif
...
void MainWindow::netUpdateServerProgressRead() {
  netServerReceiveBytes += qint32(netTcpServerConnection->bytesAvailable());
...
void MainWindow::netDisplayErrorServer(
...
void MainWindow::netDisplayErrorClient(
    QAbstractSocket::SocketError socketError) {
  if (socketError == QTcpSocket::RemoteHostClosedError) {
    netTcpClient.close();
  ...
  QMessageBox::information(
      ...
      tr("The following error occurred: %1.").arg(netTcpClient.errorString()));
  netTcpClient.close();
  netClientStrConnect = QString("未连接");
  ui->pushButtonClientTogger->setEnabled(true);
}
...
void MainWindow::on_pushButtonSendData_clicked() { comSendDataFromTextEdit(); }
...
void MainWindow::on_pushButtonNetServerTogger_clicked() {
  if (ui->pushButtonNetServerTogger->text() == "开启监听") {
    if (ui->comboBoxNetServerAddress->currentText() == QString("自定义"))
      return;
    if (netTcpServer.listen(
            ui->comboBoxNetServerAddress->currentText() == "Any"
                ? (QHostAddress::Any)
                : QHostAddress(ui->comboBoxNetServerAddress->currentText()),
...
void MainWindow::on_pushButtonClientTogger_clicked() {
  if (ui->pushButtonClientTogger->text() == "打开连接") {
    netTcpClient.connectToHost(ui->comboBoxNetClientAddress->currentText(),
